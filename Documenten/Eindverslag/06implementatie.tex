...
Deze waren handig voor het brainstormen en als iemand vast liep, kon het probleem getekend en uitgelegd worden. 
...
Voordat we konden beginnen met het database systeem, hebben we eerst een database model gemaakt. De eerste versie van dit model is te zien als Figuur \ref{brainstorm_klassediagram}.
De uiteindelijke versie van dit model is te zien als Figuur \ref{databasediagram}.
...
\begin{description}
  ...
  De Patient die echter aan een OriginalImage hing, is uiteindelijk gesneuveld, omdat dit eigenlijk niets toevoegde aan het systeem.
  Ook hebben we besloten om EditedImages niet op te slaan, en in plaats daarvan States te bewaren.
  \item[States en Projectfiles] Deze States zijn samen met de Projectfiles nieuw aan het model toegevoegd.
  ...
  \item[Users] In het originele model was er bedacht dat er voor de Users eigenlijk niets nodig zou zijn, omdat het user-management door Django geregeld zou worden.
  Zoals tijdens de implementatie is gebleken, was dit helaas niet waar, en moesten we er zelf ook nog een UserProfile aan toe voegen.
  In dit UserProfile staat opgeslagen welke User welke rechten heeft op welk project.
\end{description}
Een aantal andere aspecten van het database systeem staan hieronder uitgewerkt.
...
Om een grote verzameling projecten overzichtelijk te houden, was het noodzakelijk om een aantal trefwoorden (tags) aan een project te kunnen koppelen.
Ook moest het mogelijk zijn om onderlinge verbanden tussen projecten te leggen, door middel van dezelfde tags.
Op deze manier kan er dan makkelijk gezocht worden op de tags, en worden alleen de overeenkomstige projecten ook getoond.
Bij het aanmaken van een project kan er al gekozen worden uit de selectie met bestaande tags, zodat een project direct makkelijk gevonden kan worden.
Voor het aanmaken van nieuwe tags is het wel nodig om het project te openen.
Deze nieuwe tags kunnen daarna wel direct aan andere projecten worden gekoppeld.
